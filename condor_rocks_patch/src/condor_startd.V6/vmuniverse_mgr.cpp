 ...
 *  				pragma_boot command.
 *
 *  File:			vmuniverse_mgr.cpp
 *  Modification: 	add code block to handle rocks
 *
...
VMUniverseMgr::publish( ClassAd* ad, amask_t  /*mask*/ )
{
	if( !ad ) {
		//[PRAGMA] dprintf
		dprintf(D_ALWAYS, "[PRAGMA] VMUniverseMgr::publish returns, !ad");
		return;
	}
	if( !m_starter_has_vmcode || ( m_vm_type.Length() == 0 )) {
		ad->Assign(ATTR_HAS_VM, false);
		//[PRAGMA] dprintf
		dprintf(D_ALWAYS, "[PRAGMA] VMUniverseMgr::publish, m_starter_has_vmcode = %s, m_vm_type.Length() = %d\n", m_starter_has_vmcode ? "true" : "false", m_vm_type.Length());
		return;
	}
...
VMUniverseMgr::testVMGahp(const char* gahppath, const char* vmtype)
{
	...
	if( !m_starter_has_vmcode ) {
		//[PRAGMA] dprintf
		dprintf(D_ALWAYS, "[PRAGMA] VMUniverseMgr::testVMGahp returns false, m_starter_has_vmcode = %s\n", m_starter_has_vmcode ? "true" : "false");
		return false;
	}

	if( !gahppath || !vmtype ) {
		//[PRAGMA] dprintf
		dprintf(D_ALWAYS, "[PRAGMA] VMUniverseMgr::testVMGahp returns false, gahppath = %s, vmtype=%s\n", gahppath , vmtype);
		return false;
	}
	...
	if( !read_something ) {
		...
		// [PRAGMA] add rocks else-if block
		else if( strcasecmp(vmtype, CONDOR_VM_UNIVERSE_ROCKS ) == 0 ) {
					MyString err_msg;
					MyString err_msg2;
					err_msg += "\n#######################################################\n";
					err_msg += "##### Make sure the followings ";
					err_msg += "to use VM universe for Rocks\n";

					if( can_switch_ids() ) {
						// Condor runs as root
						err_msg += "### - The script program like 'condor_vm_rocks'";
						err_msg += " must be readable for anybody.\n";
					}

					err_msg += "### - Check the path of rockscmd, vmrun, and mkisofs ";
					err_msg += "in 'condor_vm_rocks\n'";
					err_msg += "#########################################################\n";
					dprintf( D_ALWAYS, "%s", err_msg.Value());
				}
		//[PRAGMA] dprintf
		dprintf(D_ALWAYS, "[PRAGMA] VMUniverseMgr::testVMGahp returns false, read_something = %s\n", read_something ? "true" : "false");
		return false;
	}
	...
	//[PRAGMA] dprintf
	dprintf(D_ALWAYS, "[PRAGMA] VMUniverseMgr::testVMGahp, m_vm_type = %s,  m_vmgahp_server = %s\n", m_vm_type.Value(), m_vmgahp_server.Value());
...
VMUniverseMgr::canCreateVM(ClassAd *jobAd)
{
	if( !m_starter_has_vmcode || ( m_vm_type.Length() == 0 )) {
		//[PRAGMA] dprintf
		dprintf(D_ALWAYS, "[PRAGMA] VMUniverseMgr::canCreateVM, m_starter_has_vmcode = %s, m_vm_type.Length() = %d\n", m_starter_has_vmcode ? "true" : "false", m_vm_type.Length());
		return false;
	}

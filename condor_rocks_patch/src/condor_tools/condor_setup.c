 ...
 *
 *  File:			condor_setup.c
 *  Modification: 	Add a code block to get ROCKS_PERL and ROCKS_SCRIPT (optional modification)
 *
...
set_vmgahpoptions() {
	...
	if ( Opt.perllocation ) {
		...
		set_option("ROCKS_PERL", Opt.perllocation);
	...
	if ( Opt.release_dir ) {
		...
		// [PRAGMA] It's not necessary to add the following block
		char *control_script = "bin\\condor_vm_rocks";
		...
		if (tmp) {
			...
			set_option("ROCKS_SCRIPT", short_name);
			free(short_name);
		}
		// [PRAGMA] End of block

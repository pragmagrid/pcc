 ...
 *
 *  File:			submit.cpp
 *  Modification: 	Add a code block to process rocks vm control
 *
...
SetVMParams()
{
	...
	//[PRAGMA] add rocks specific code
	else if( strcasecmp(VMType.Value(), CONDOR_VM_UNIVERSE_ROCKS) == MATCH ) {
		bool rocks_should_transfer_files = false;
		tmp_ptr = condor_param("rocks_should_transfer_files");
		if( parse_vm_option(tmp_ptr, rocks_should_transfer_files) == false ) {
			MyString err_msg;
			err_msg = "\nERROR: You must explicitly specify "
				"\"rocks_should_transfer_files\" "
				"in your submit description file. "
				"You need to define either: "
		...
		buffer.formatstr( "%s = %s", VMPARAM_ROCKS_TRANSFER,
				rocks_should_transfer_files ? "TRUE" : "FALSE");
		InsertJobExpr( buffer );

		if( rocks_should_transfer_files == false ) {
			vm_need_fsdomain = true;
		}

		/* In default, rocks vm universe uses snapshot disks.
		 * However, when a user job is so IO-sensitive that
		 * the user doesn't want to use snapshot,
		 * the user must use both
		 * rocks_should_transfer_files = YES and
		 * rocks_snapshot_disk = FALSE
		 * In rocks_should_transfer_files = NO
		 * snapshot disks is always used.
		 */
		bool rocks_snapshot_disk = true;
		tmp_ptr = condor_param("rocks_snapshot_disk");
		if( tmp_ptr ) {
			parse_vm_option(tmp_ptr, rocks_snapshot_disk);
			free(tmp_ptr);
		}

		if(	!rocks_should_transfer_files && !rocks_snapshot_disk ) {
			MyString err_msg;
			err_msg = "\nERROR: You should not use both "
				"rocks_should_transfer_files = FALSE and "
				"rocks_snapshot_disk = FALSE. "
				"Not using snapshot disk in a shared file system may cause "
				"problems when multiple jobs share the same disk\n";
			print_wrapped_text( err_msg.Value(), stderr );
		...
		buffer.formatstr( "%s = %s", VMPARAM_ROCKS_SNAPSHOTDISK,
		...
		if ( rocks_job_dir ) {
			...
			dir.Rewind();
			while ( dir.Next() ) {
				if ( has_suffix( dir.GetFullPath(), ".vmconf" ) ||
						rocks_should_transfer_files ) {
					// The .vmconf file is always transfered.
					transfer_vm_file( dir.GetFullPath() );
				}
			}
		}

		// Look for .vmconf and .img files in transfer_input_files
		StringList vmconf_files;
		StringList img_files;
		StringList input_files( NULL, "," );
		MyString input_files_str;
		job->LookupString( ATTR_TRANSFER_INPUT_FILES, input_files_str );
		input_files.initializeFromString( input_files_str.Value() );
		...
		const char *file;
		while ( (file = input_files.next()) ) {
			if ( has_suffix( file, ".vmconf" ) ) {
				vmconf_files.append( condor_basename( file ) );
			} else if ( has_suffix( file, ".img" ) ) {
				img_files.append( condor_basename( file ) );
			}
		}

		if ( vmconf_files.number() == 0 ) {
			fprintf( stderr, "\nERROR: no vmconf file for rocks can be found.\n" );
			DoCleanup(0,0,NULL);
			exit(1);
		} else if ( vmconf_files.number() > 1 ) {
			fprintf( stderr, "\nERROR: multiple vmconf files exist. "
					 "Only one vmconf file should be present.\n" );
			DoCleanup(0,0,NULL);
			exit(1);
		} else {
			vmconf_files.rewind();
			buffer.formatstr( "%s = \"%s\"", VMPARAM_ROCKS_VMCONF_FILE,
					condor_basename(vmconf_files.next()));
			InsertJobExpr( buffer );
		}

		tmp_ptr = img_files.print_to_string();
		if ( tmp_ptr ) {
			buffer.formatstr( "%s = \"%s\"", VMPARAM_ROCKS_IMG_FILES, tmp_ptr);
			InsertJobExpr( buffer );
			free( tmp_ptr );
		}
	}

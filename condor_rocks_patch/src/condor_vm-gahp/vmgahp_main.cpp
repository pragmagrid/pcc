 ...
 *  				pragma_boot command.
 *
 *  File:			vmgahp_main.cpp
 *  Modification:	Included rocks_type.h, and added rocks specified condition code blocks
 *
...
void main_init(int argc, char *argv[])
{
	...
	//[PRAGMA] Check parameters for rocks
	else if( strcasecmp(vmtype.Value(), CONDOR_VM_UNIVERSE_ROCKS) == 0 ) {
		priv_state priv = set_user_priv();
		if( RocksType::checkRocksParams(gahpconfig) == false ) {
			DC_Exit(1);
		}
		set_priv(priv);
	}
	...
	if( vmgahp_mode == VMGAHP_TEST_MODE ) {
	  ...
	  if( (strcasecmp(vmtype.Value(), CONDOR_VM_UNIVERSE_XEN) == 0)) {
		...
		//[PRAGMA] Test mode for Rocks
		else if( strcasecmp(vmtype.Value(), CONDOR_VM_UNIVERSE_ROCKS) == 0 ) {
			priv_state priv = set_user_priv();
			if( RocksType::testRocks(gahpconfig) == false ) {
				vmprintf(D_ALWAYS, "\nERROR: the vm_type('%s') cannot "
						"be used.\n", vmtype.Value());
				DC_Exit(0);
			}
	...
	if( vmgahp_mode == VMGAHP_KILL_MODE ) {
		...
		//[PRAGMA] Kill Rocks VM
		else if( strcasecmp(vmtype.Value(), CONDOR_VM_UNIVERSE_ROCKS ) == 0 ) {
			RocksType::killVMFast(gahpconfig->m_prog_for_script.Value(),
					gahpconfig->m_vm_script.Value(), matchstring.Value(), true);
		}
		DC_Exit(0);
	}
